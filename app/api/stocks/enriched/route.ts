...
export async function GET(request: NextRequest) {
  try {
    ...
    const hierarchyLevel = searchParams.get("hierarchyLevel") as
      | "Low"
      | "Moderate"
      | "High"
      | null;
    ...
    const aiTier = searchParams.get("aiTier")
      ? parseInt(searchParams.get("aiTier")!)
      : null;
    ...
    if (minScore !== null) {
      enriched = enriched.filter(
        (s) => s.scores && s.scores.composite >= minScore,
      );
    }

    if (maxScore !== null) {
      enriched = enriched.filter(
        (s) => s.scores && s.scores.composite <= maxScore,
      );
    }
